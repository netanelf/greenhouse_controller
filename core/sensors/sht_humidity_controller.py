...
class ShtHumidityController(SensorController):
    def __init__(self, name: str, sht_driver: ShtDriver, simulate: bool = True):
        super(ShtHumidityController, self).__init__(name)
        self._logger.info('initializing ShtHumidityController')
    ...
    def read(self) -> Measurement:
        if self._simulate:
            ...
            h = self._sht_driver.get_humidity()
            if h is None:
