...
def populate_sensors():
    ...
    Sensor.objects.get_or_create(name='dht22_temp_window', kind=dht_22_temp, simulate=True, pin=15, i2c=False)[0]

    print 'creating sensor: {}'.format('dht22_humidity_window')
    Sensor.objects.get_or_create(name='dht22_humidity_window', kind=dht_22_humidity, simulate=True, pin=15, i2c=False)[0]

    print 'creating sensor: {}'.format('DS18B20_water')
    Sensor.objects.get_or_create(name='DS18B20_water', kind=ds18b20, simulate=True, pin=99, i2c=False, device_id='28-031467d282ff')[0]

    print 'creating sensor: {}'.format('DS18B20_indoor')
    Sensor.objects.get_or_create(name='DS18B20_indoor', kind=ds18b20, simulate=True, pin=99, i2c=False, device_id='28-031467eefbff')[0]

    print 'creating sensor: {}'.format('TSL2561_lux_1')
    Sensor.objects.get_or_create(name='lux_1', kind=tsl2561, simulate=True, pin=99, i2c=True, device_id='0x39')[0]


def populate_relays():

    t = TimeGovernor.objects.get_or_create(name='light1', kind='O', on_start_time='21:55:00', on_end_time='22:00:00', recurring_on_start_time='08:00:00', recurring_on_period=60, recurring_off_period=30)[0]
